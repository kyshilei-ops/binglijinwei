...
export default function BlogPostPage() {
  ...
  if (!post) {
    return (
      <>
        <Header />
        <LocalizedPageMetadata titleZh="博客" titleEn="Blog" />
  ...
  const postAuthor = localizedAuthor(post.author, lang);
  ...
  return (
    <>
      ...
      <LocalizedPageMetadata
        ...
        descriptionEn={post.excerpt_en || post.excerpt}
      />
      <main className="flex-1">
        <section className="py-12">
          <div className="container max-w-3xl">
            ...
            <span className="inline-block bg-[#4caf50] text-white text-xs font-medium px-3 py-1 rounded mb-4">{localizedBlogCategory(post.category, post.category_en, lang)}</span>
            ...
            <div className="blog-content max-w-none text-[#4a5568] leading-relaxed text-lg">
              {postContent ? (
                ...
                <>
                  <p>{lang === "zh" ? post.excerpt : (post.excerpt_en || post.excerpt)}</p>
                </>
              )}
            </div>
          </div>
